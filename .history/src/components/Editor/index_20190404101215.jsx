...
export default class Editor extends React.Component {
    ...
    instanceReady = () => {
        ...
        this.autocomplete.getHtmlToInsert = function (item) {
            setTimeout(() => {
                ...
                // }
                if (that.range) {
                    that.range.endContainer.$.parentNode.className = 'temporary';
                }
                if (item.charts) {
    ...
    textTestCallback = (range) => {
        ...
        return window.CKEDITOR.plugins.textMatch.match(range, (txt, offset) => {
            ...
            if (range.startContainer.$.parentNode.className.indexOf('temporary') !== -1) {　//选择公司后进入此逻辑
                ...
                if (matchArr.length > 1) {
                    ...
                    if (lastText.indexOf('归母晶') !== -1) {
                        ...
                        range.endContainer.$.parentNode.className = 'temporary';
                    }
                    return {
                        start: text.lastIndexOf('.') + 1,
                        end: text.length
